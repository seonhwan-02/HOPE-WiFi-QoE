...
print("=" * 50)
print("DE 실행 중 (Differential Evolution)...")
...
W_RTT_DE  = w_rtt_raw / t
W_UTIL_DE = w_util_raw / t

print()
print("=" * 50)
print("DE 결과")
print("=" * 50)
print(f"  RTT 가중치:       {W_RTT_DE:.4f}  ({W_RTT_DE*100:.1f}%)")
print(f"  채널사용률 가중치: {W_UTIL_DE:.4f}  ({W_UTIL_DE*100:.1f}%)")
print(f"  수렴 성공 여부:    {result.success}")
print(f"  목적함수 최솟값:   {result.fun:.6f}  (상관계수: {1-result.fun:.6f})")
print()

# ── 기존 수동 60:40과 성능 비교 ──
score_de     = (W_RTT_DE  * P_RTT + W_UTIL_DE  * P_UTIL)
score_manual = (0.60      * P_RTT + 0.40        * P_UTIL)

corr_de     = np.corrcoef(score_de,     y_true)[0,1]
corr_manual = np.corrcoef(score_manual, y_true)[0,1]

print("=" * 50)
print("DE 도출 vs 수동 60:40 성능 비교 (혼잡 샘플)")
print("=" * 50)
print(f"  DE 도출  ({W_RTT_DE*100:.1f}%:{W_UTIL_DE*100:.1f}%): 상관계수 = {corr_de:.4f}")
print(f"  수동 60:40:                    상관계수 = {corr_manual:.4f}")
print(f"  차이: {(corr_de - corr_manual)*100:.2f}%p")
