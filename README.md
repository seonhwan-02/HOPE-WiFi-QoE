...
4. **차분진화 (Differential Evolution):** 혼잡 환경에서의 최적 가중치(RTT vs Util)를 scipy의 `differential_evolution`으로 산출.

---

## 📊 측정 데이터 예시

`wifi_data.xlsx` / `wifi_data2.xlsx`에는 RSSI, 채널사용률, RTT(평균지연시간), 패킷손실률, 업/다운로드 실측 속도 등이 샘플별로 기록되어 있습니다.

![Data Sample](results/data_sample.png)
...
│   ├── de_weight_finder.py   # Differential Evolution 기반 가중치 탐색
...
# 가중치 탐색 (Differential Evolution) 실행
python src/de_weight_finder.py
